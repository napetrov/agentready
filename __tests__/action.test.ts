...
describe('runAction (scan mode)', () => {
  ...
  it('leaves score/failed unchanged under the default policy', async () => {
    const withoutPolicy = await run({ path: badFixture, failOnSeverity: 'off' })
    const withDefault = await run({ path: badFixture, failOnSeverity: 'off', policy: 'default' })
    expect(withDefault.score).toBe(withoutPolicy.score)
    expect(withDefault.policyAdjustmentsCount).toBe(0)
    expect(withDefault.policyEffectiveScore).toBeUndefined()
    expect(withDefault.summaryMarkdown).not.toContain('Policy:')
  })

  it('applies enterprise policy adjustments to gating, score, and the summary without changing the raw score', async () => {
    const raw = await run({ path: badFixture, failOnSeverity: 'off' })
    const enterprise = await run({ path: badFixture, failOnSeverity: 'off', policy: 'enterprise' })
    expect(enterprise.score).toBe(raw.score) // score stays the raw, policy-independent value
    expect(enterprise.policyAdjustmentsCount).toBeGreaterThan(0)
    expect(enterprise.policyEffectiveScore).toBeLessThanOrEqual(raw.score)
    expect(enterprise.summaryMarkdown).toContain('Policy: enterprise')
  })
