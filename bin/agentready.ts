 ...
 * module load) so tests can drive each subcommand in-process and assert on
 * output/exit codes; the bin only parses `process.argv` when run directly.
 */
...
// Run only when invoked directly (not when imported by tests). parseAsync (not
// parse) so rejections from the async `analyze` action handler are caught here
// and set a non-zero exit code, rather than becoming an unhandled rejection.
