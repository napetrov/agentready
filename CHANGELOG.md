...
### Added
- `.gitignore` parity in the file inventory: the repository walk (`detectors/file-inventory.ts`) now honours the repository's own `.gitignore` files — root and nested, including negations — with the same hierarchy semantics git uses, so build output, caches, and other ignored paths are no longer inventoried or flagged. The always-ignored directory set (`.git`, `node_modules`, `dist`, `build`, `out`, `coverage`, `.next`, `.turbo`, `.vercel`) still applies on top, and explicit `ignorePaths` config is unchanged. Reading `.gitignore` files is the only added filesystem access; nothing is executed.
